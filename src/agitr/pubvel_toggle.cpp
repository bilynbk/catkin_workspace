// This program toogles(swich) between rotation and translation commands,
...
bool forward = true;
// write a service callback "toggleForward"
...
int main(int argc, char **argv) {
	...
	ros::ServiceServer server = nh.advertiseServer("toggle_forward", &toggleForward);
	
	//Publish commands, using the latest value for forward,until the node shuts down.
	ros:Publisher pub = nh.advertise<geometry_msgs::Twist>("turtle1/cmd_vel",1000);
	ros::Rate rate(2);
	while(ros::ok()) {
		geometry_msgs::Twist msg; // define message name "msg"
